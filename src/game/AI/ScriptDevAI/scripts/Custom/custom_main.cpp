...
enum
{
    NPC_TEXT_NAMECHANGER = 42313,
};

static bool GossipHello_namechanger(Player *player, Creature *creature)
{
    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_CHAT, "Change my name", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1, "Input new name and click Accept button.", 0, true);
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Nevermind", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
    player->SEND_GOSSIP_MENU(NPC_TEXT_NAMECHANGER, creature->GetObjectGuid());
    return true;
}

static bool GossipSelect_namechanger(Player* player, Creature* creature, uint32 sender, uint32 action)
{
...
static bool GossipSelectWithCode_namechanger(Player *player, Creature *creature, uint32 sender, uint32 action, const char* sCode)
{
    uint8 res = ObjectMgr::CheckPlayerName(sCode, true);
    if (res != CHAR_NAME_SUCCESS)
    {
        player->GetSession()->SendNotification("Name has invalid structure/symbols.");
        GossipHello_namechanger(player, creature);
        return false;
    }

    if (player->GetSession()->GetSecurity() == SEC_PLAYER && sObjectMgr.IsReservedName(sCode))
    {
        player->GetSession()->SendNotification("Name is reserved.");
        GossipHello_namechanger(player, creature);
        return false;
    }

    std::string escaped_newname = sCode;
    CharacterDatabase.escape_string(escaped_newname);

    if (!normalizePlayerName(escaped_newname))
    {
        player->GetSession()->SendNotification("Invalid new name.");
        GossipHello_namechanger(player, creature);
        return false;
    }

    bool result = CharacterDatabase.AsyncPQuery(WorldSession::HandleChangePlayerNameOpcodeCallBack,
        player->GetSession()->GetAccountId(), escaped_newname,
        "SELECT guid, name FROM characters WHERE guid = %u AND account = %u AND NOT EXISTS (SELECT NULL FROM characters WHERE name = '%s')",
        player->GetDbGuid(), player->GetSession()->GetAccountId(), escaped_newname.c_str()
    );

    if (result)
    {
        player->SetName(escaped_newname);
        WorldPacket data(SMSG_FORCE_DISPLAY_UPDATE, 8);
        data << player->GetObjectGuid();
        player->GetSession()->SendPacket(data, true);
        GossipHello_namechanger(player, creature);
...
void AddSC_custom_main()
{
    ...
    newscript = new Script;
    newscript->Name = "namechanger";
    newscript->pGossipHello = &GossipHello_namechanger;
    newscript->pGossipSelect = &GossipSelect_namechanger;
    newscript->pGossipSelectWithCode = &GossipSelectWithCode_namechanger;
    newscript->RegisterSelf();
